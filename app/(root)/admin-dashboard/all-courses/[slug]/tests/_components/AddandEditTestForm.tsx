...
import { useGetTestById } from "@/hooks/useGetTestsByCourseSlug";
...
function AddandEditTestForm({ isEdit = false }: Props) {
  ...
  const { test, isLoading: isLoadingTest } = useGetTestById(test_id);
  ...
  const { mutate, isPending } = useMutation({
    mutationKey: [QUERY_KEYS.testsByCourseSlug, course_slug],
    mutationFn: async (data: TestFormValues) => {
      ...
      if (isEdit) {
        ...
        const response = await customAxios.post(
          `admin/courses/${course_slug}/tests`,
          values
        );
        return response.data;
      }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.testsByCourseSlug, course_slug],
      });
      toast.success(
        `Test muvaffaqiyatli ${isEdit ? "yangilandi" : "qo'shildi"}`
      );
      router.push(`/admin-dashboard/all-courses/${course_slug}/tests`);
    },
  });
...
export default AddandEditTestForm;
