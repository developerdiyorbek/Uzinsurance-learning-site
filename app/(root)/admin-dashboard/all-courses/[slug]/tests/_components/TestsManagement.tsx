...
import { useGetTestsByCourseSlug } from "@/hooks/useGetTestsByCourseSlug";
...
export default function TestsManagement() {
  ...
  const { tests, isLoading, course, testsCount } =
    useGetTestsByCourseSlug(course_slug);
  ...
  const { mutate: updateOrder, isPending: isUpdatingOrder } = useMutation({
    ...
    mutationFn: async (updates: { test_id: string; order: number }[]) => {
      await customAxios.put(`admin/courses/${course_slug}/tests/reorder`, {
        updates,
      });
    },
    onSuccess: () => {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`/admin-dashboard/all-courses/${course_slug}/tests/add`}>
